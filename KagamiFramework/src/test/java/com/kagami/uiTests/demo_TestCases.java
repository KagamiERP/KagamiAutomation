...
public class demo_TestCases {
	...
	public void register() throws Exception {
		...
		driver = testPreconditions.browserType(driver, Global.sBrowserName);
		
		/*Map<String, List<String>> registerForm = genericMethods.readMultipleTestData(Global.sTestData, "Sheet1");
		for(int iRowCounter = 1; iRowCounter<registerForm.size();iRowCounter++){
			List<String> listData = new ArrayList<String>();
			for(int iColumnCounter=0; iColumnCounter< registerForm.size(); iColumnCounter++){
				String value = listData.get(iColumnCounter);
			}
		}
		*/
		...
		//genericMethods.clickElement(driver, By.name("register"));
